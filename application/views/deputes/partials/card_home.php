<div class="card card-depute mx-2">
  ...
  <?php endif; ?>
  <div class="card-avatar-depute<?= $depute['legislature'] >= 17 ? '-square' : '-circle' ?> card-avatar">
    <?php if ($depute['img'] && $this->config->item('mp_photos')): ?>
      <img class="img-lazy" src="<?= asset_url() ?>imgs/placeholder/placeholder-face.png" width="130" height="166" data-src="<?= base_url(); ?>assets/imgs/<?= $depute['legislature'] >= 17 ? "deputes_original/" : "deputes_nobg/" ?>depute_<?= substr($depute["mpId"], 2) ?>.png" alt="<?= $depute['nameFirst'].' '.$depute['nameLast'] ?>">
    <?php else: ?>
