    ...
    <div class="row">
      <?php if ($no_participation) : ?>
        <div class="col-12 mt-2">
          <p>Il n'y a pas encore eu suffisamment de votes solennels à l'Assemblée nationale pour afficher cette statistique. Pour consulter ma participation à l'ensemble des scrutins, <a href="<?= base_url() ?>statistiques/deputes-participation">cliquez ici</a>.</p>
        </div>
      <?php else : ?>
          ...
          <div class="texte ml-md-3 pl-md-3 mt-md-0 mt-3">
            <?php if ($depute['legislature'] == legislature_current()): ?>
              <p>
                ...
                <?php else : ?>
                  Quand j’étais en activité à l'Assemblée,
                <?php endif; ?>
                j’ai participé à <?= $participation['score'] ?>% des votes solennels à l’Assemblée nationale.
              </p>
              <p>
                Je vote <?= $active ? "" : "ais" ?> <b><?= $edito_participation['all'] ?></b> que la moyenne des députés, qui est <?= $edito_participation['all'] == "autant" ? "également" : "" ?> de <?= $participation['all'] ?>%.
              </p>
              <?php if ($participation['group']): ?>
                <p>
                  De plus, je vote <?= $active ? "" : "ais" ?> <b><?= $edito_participation['group'] ?></b> que la moyenne des députés de mon groupe politique, qui est <?= $edito_participation['group'] == "autant" ? "également" : "" ?> de <?= $participation['group'] ?>%.
                </p>
              <?php endif; ?>
            <?php else: ?>
              <p>
                Pendant la <?= $depute['legislature'] ?><sup>e</sup> législature, j’ai participé à <?= $participation['score'] ?>% des votes solennels à l’Assemblée nationale.
              </p>
              <p>
                Je votais <b><?= $edito_participation['all'] ?></b> que la moyenne des députés, qui était <?= $edito_participation['all'] == "autant" ? "également" : "" ?> de <?= $participation['all'] ?>%.
              </p>
              <?php if ($participation['group']): ?>
                <p>
                  De plus, je votais <b><?= $edito_participation['group'] ?></b> que la moyenne des députés de mon groupe politique, qui était <?= $edito_participation['group'] == "autant" ? "également" : "" ?> de <?= $participation['group'] ?>%.
                </p>
              <?php endif; ?>
            <?php endif; ?>
            <p>
