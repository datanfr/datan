      <div class="row mt-4">
        <div class="col-md-10">
          ...
          <p><?= ucfirst($mpOldestGender['le']) ?> <?= $mpOldestGender['depute'] ?> en activité le plus âgé<?= $mpOldestGender['e'] ?> est <a href="<?= base_url() ?>deputes/<?= $mpOldest['dptSlug'] ?>/depute_<?= $mpOldest['nameUrl'] ?>" target="_blank"><?= $mpOldest['name'] ?></a> (<?= $mpOldest['age'] ?> ans), membre du groupe <?= $mpOldest["groupLibelle"] ?>.</p>
          <p>
            ...
            <?= ucfirst($mpYoungestGender['pronom']) ?> est membre du groupe <?= $mpYoungest['groupLibelle'] ?>.
          </p>
        </div>
      </div>
      <div class="row row-grid mt-5">
        <div class="col-md-6 py-3">
          ...
          <div class="d-flex justify-content-center">
            <?php $this->load->view('deputes/partials/card_home.php', array('depute' => $mpOldest, 'tag' => 'span', 'stats' => $mpOldest['age'] . " ans", 'cat' => false)) ?>
          </div>
        </div>
        <div class="col-md-6 py-3">
          <h2 class="text-center"><?= ucfirst($mpYoungestGender['le']) ?> <?= $mpYoungestGender['depute'] ?> <?= $mpYoungestGender["le"] ?> plus jeune</h2>
          <div class="d-flex justify-content-center">
            <?php $this->load->view('deputes/partials/card_home.php', array('depute' => $mpYoungest, 'tag' => 'span', 'stats' => $mpYoungest['age'] . " ans", 'cat' => false)) ?>
          </div>
        </div>
      </div>
      <div class="mt-5">
