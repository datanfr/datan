      ...
      initComplete: function () {
          this.api().columns().every( function (i) {
            ...
            if (screen.width > 767) {
              if (i == 1 || i == 2 ||- i == 3) {
                $(column.header()).append("<br>");
                const select = $('<select style="margin-top:12px; margin-bottom:5px" class="form-control" aria-label="Filtrer par ' + $(column.header()).text().trim() + '"><option value=""></option></select>')
                    .appendTo( $(column.header()) )
                    .on( 'change', function () {
