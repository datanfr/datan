...
<html lang="en" dir="ltr">
  ...
  This script cleans the 'deputes' table by keeping in the table only the
  rows uploaded by the previous file (1_deputes.php).
    ...
    ?>
        ...
        <div class="col">
            ...
            while ($data = $reponse->fetch()) {
              echo '<p>'.$i.'. '.$data['mpId'].' ==> '.$data['nameFirst'].' '.$data['nameLast'].'</p>';
              $i++;
            }
            if (isset($dateMaj)) {
              $bdd->exec('DELETE FROM deputes WHERE dateMaj < "'.$dateMaj.'" OR dateMaj IS NULL');
