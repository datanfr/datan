<?php
  ...
  /*
    With this file, the new version of the database will be => 5
  ...
  */
  ...
  if ($new_v > $last_v_installed) {
    ...
    /* 2. MAKE YOUR CHANGE TO THE DATABASE */
    if (!count($bdd->query("SHOW COLUMNS FROM `amendements` LIKE 'examined_by'")->fetchAll())) {
      $bdd->query('ALTER TABLE `amendements` ADD `examined_by` VARCHAR(255) NULL DEFAULT NULL AFTER `sort`');
    }
